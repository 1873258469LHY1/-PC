<template>
  <div class="spec-preview">
    <img :src="skuInfo.skuDefaultImg" />
    <div class="event" @mousemove="bigShow"></div>
    <div class="big">
      <img
        :src="skuInfo.skuDefaultImg"
        :style="{ top: ttop + 'px', left: tleft + 'px' }"
      />
    </div>
    <div class="mask" :style="{ top: ztop + 'px', left: zleft + 'px' }"></div>
  </div>
</template>

<script>
import { mapGetters } from "vuex";
export default {
  name: "Zoom",
  data() {
    return {
      // mask的定位
      zleft: 0,
      ztop: 0,
      //   big中图片的定位
      tleft: 0,
      ttop: 0,
    };
  },
  methods: {
    bigShow(e) {
      // mask定位
      this.zleft = e.offsetX - 100;
      this.ztop = e.offsetY - 100;
      //   放大镜界限判断
      this.zleft = this.zleft > 200 ? 200 : this.zleft;
      this.zleft = this.zleft < 0 ? 0 : this.zleft;
      this.ztop = this.ztop > 200 ? 200 : this.ztop;
      this.ztop = this.ztop < 0 ? 0 : this.ztop;
      // big中大图定位
      this.tleft = -2 * this.zleft;
      this.ttop = -2 * this.ztop;
    },
  },
  computed:{
      ...mapGetters(['skuInfo'])
  }
};
</script>

<style lang="less">
.spec-preview {
  position: relative;
  width: 400px;
  height: 400px;
  border: 1px solid #ccc;

  img {
    width: 100%;
    height: 100%;
  }

  .event {
    width: 100%;
    height: 100%;
    position: absolute;
    top: 0;
    left: 0;
    z-index: 998;
  }

  .mask {
    width: 50%;
    height: 50%;
    background-color: rgba(0, 255, 0, 0.3);
    position: absolute;
    left: 0;
    top: 0;
    display: none;
  }

  .big {
    width: 100%;
    height: 100%;
    position: absolute;
    top: -1px;
    left: 100%;
    border: 1px solid #aaa;
    overflow: hidden;
    z-index: 998;
    display: none;
    background: white;

    img {
      width: 200%;
      max-width: 200%;
      height: 200%;
      position: absolute;
      left: 0;
      top: 0;
    }
  }

  .event:hover ~ .mask,
  .event:hover ~ .big {
    display: block;
  }
}
</style>